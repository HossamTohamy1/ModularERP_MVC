    ...
    public class TreasuryService : ITreasuryService
    {
        ...
        public async Task<TreasuryViewModel> CreateAsync(TreasuryViewModel viewModel)
        {
            try
            {
                ...
                if (!currencyExists)
                {
                    throw new ArgumentException($"Currency not found or inactive");
                }

                // Check for duplicate name
                if (await IsDuplicateNameAsync(viewModel.CompanyId, viewModel.Name))
                {
                    throw new InvalidOperationException($"A treasury with the name '{viewModel.Name}' already exists in this company");
                }
            ...
            try
            {
                ...
                if (hasVouchers)
                {
                    throw new InvalidOperationException(
                        "Cannot delete treasury with associated transactions. You can deactivate it instead");
                }
